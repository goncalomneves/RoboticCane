...
% figure('Renderer', 'painters', 'Position', [10 10 480 400]);
% % figure('Renderer', 'painters', 'Position', [10 10 960 540]);
% pzmap(sys_ang_ctr3);
...
Simulation_Time = 30;
sim('wheel_and_bar10',Simulation_Time);
...
% max_pos = [max_pos max(abs(y(:,1)))];
% max_ang = [max_ang max(abs(y(:,2)))];
...
% figure('Renderer', 'painters', 'Position', [10 10 960 540]);
yyaxis left
% plot(t,y(:,1),'-k',t,ref(:,1),'--k','LineWidth',1.2);
plot(t,y(:,1),t,ref(:,1));
ylabel('Position[m]');
yyaxis right
% plot(t,y(:,2),':k',t,ref(:,2),'-.k','LineWidth',1.2);
plot(t,y(:,2),t,ref(:,2));
ylabel('Angle[rad]');
title({'State of the rod'; ['alpha = ', num2str(alpha)]});
xlabel('Time[s]');
l = legend('$x[m]$','$x_{ref} [m]$','$\theta [rad]$','$\theta_{ref} [rad]$','Location', 'Southeast');
set(l, 'Interpreter', 'latex');

figure('Renderer', 'painters', 'Position', [10 10 300 200]);
% figure('Renderer', 'painters', 'Position', [10 10 960 540]);
% plot(t,f(:,1),'-k',t,f(:,2),'--k','LineWidth',1.2);
plot(t,f);
title('Force Applied');
xlabel('Time(s)');
l = legend('$F_x[N]$','$F_y[N]$','Location', 'Southeast');
set(l, 'Interpreter', 'latex');
...
% xl = xlabel('$\alpha$');
% l = legend('$RMSE_{mean}$','$RMSE_{position}$','$RMSE_{angle}$','$Max_{position} [m]$','$Max_{angle} [rad]$');
